...
import type { AssetVersion, Asset, Subscription } from '@gravyty/domain';
 ...
 */
export async function runContentHubScheduler(options: { now?: Date } = {}): Promise<SchedulerJobResult> {
  ...
  try {
    ...
    const scheduledVersions = (scheduledResult.Items || []) as AssetVersion[];
    ...
    for (const version of scheduledVersions) {
      try {
        ...
        const { version: publishedVersion, asset: assetUpdate } = publishVersion(
          version,
          'system', // System user for scheduled publishes
        ...
        try {
          ...
          if (updatedAsset.Item) {
            const asset = updatedAsset.Item as Asset;
            ...
            const subscriptions = (subscriptionResult.Items || []) as Subscription[];
    ...
    const versionsToExpire = (expireResult.Items || []) as AssetVersion[];
    ...
    for (const version of versionsToExpire) {
      try {
        ...
        const asset = assetResult.Item as Asset | undefined;
